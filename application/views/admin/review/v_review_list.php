			...
			<table class="table table-bordered list_table mt-2" width="100%" cellspacing="0">
				...
				<thead>
					<tr>
						...
						<th class="selectSort <?php if ($page_info['sort_key'] == 'score') echo ($page_info['sort_direction'] == 'desc') ? 'desc' : 'asc'; ?>" data-sort_key="score">별점</th>
						<th class="selectSort <?php if ($page_info['sort_key'] == 'state') echo ($page_info['sort_direction'] == 'desc') ? 'desc' : 'asc'; ?>" data-sort_key="state">상태</th>
					</tr>
				</thead>
				<tbody>
					<?php
					...
					?>
						<tr>
							...
							<td class="text-center"><?= replaceReviewStatus($list->status) ?></td>
						</tr>
					<?php
					}
					?>
